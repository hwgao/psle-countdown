// PSLE 2026 Subject Exam Dates (Official - Tentative)
        const examSchedule = [
            { subject: 'English & Mother Tongue', type: 'Oral', date: 'August 12, 2026 08:00:00', color: '#9b59b6' },
            { subject: 'Mother Tongue', type: 'Listening', date: 'September 15, 2026 09:00:00', color: '#ec4899' },
            { subject: 'English', type: 'Written', date: 'September 24, 2026 09:00:00', color: '#9b59b6' },
            { subject: 'Mathematics', type: 'Written', date: 'September 25, 2026 09:00:00', color: '#3b82f6' },
            { subject: 'Science', type: 'Written', date: 'September 28, 2026 09:00:00', color: '#10b981' },
            { subject: 'Mother Tongue', type: 'Written', date: 'September 29, 2026 09:00:00', color: '#ec4899' }
        ];
            ...
            const hours = Math.floor((nextExam.distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
            const minutes = Math.floor((nextExam.distance % (1000 * 60 * 60)) / (1000 * 60));
        ...
        // Update countdown every second
            ...
            function updateAuthUi() {
                ...
                if (currentUser) {
                    const display = currentUser.displayName || currentUser.email || `${currentUser.uid.slice(0, 6)}…${currentUser.uid.slice(-4)}`;
                    ...
                    authStatus.textContent = 'Sign in to see and manage your private learning plan.';
            ...
            function startTodosListener() {
                ...
                todosUnsubscribe = userTodosCollection.orderBy('createdAt', 'desc').onSnapshot((snapshot) => {
                    todos = [];
        ...
        // Render todos to DOM
            function renderTodos() {
                ...
                if (!currentUser) {
                    todoList.innerHTML = '<div style="text-align: center; padding: 2rem; opacity: 0.7;">Sign in to view and manage your private tasks.</div>';
                    todoCount.textContent = '0 tasks';
                    return;
                }

                if (todos.length === 0) {
                    todoList.innerHTML = '<div style="text-align: center; padding: 2rem; opacity: 0.6;">No tasks yet. Add your first learning goal!</div>';
                    todoCount.textContent = '0 tasks';
                    return;
            }
                    ...
                    if (error && (error.code === 'auth/user-not-found' || error.code === 'auth/invalid-login-credentials')) {
                        try {
                            ...
                            if (createError && createError.code === 'auth/email-already-in-use') {
                                setAuthError('Incorrect password for this email, or the account already exists. Try again or use Google sign-in.');
                                return;
                            }
        ...
        function showTasksForDate(dateStr) {
            ...
            popupBody.innerHTML = tasksForDate.map(todo => {
                const priorityClass = `priority-${todo.priority || 'medium'}`;
                const subjectClass = `subject-${todo.subject || 'general'}`;
